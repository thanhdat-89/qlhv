                    ...
                    {parseFloat(formData.discountRate) > 0 && (
                        <div className="animate-fade-in">
                            <label className="form-label" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                <span>Tháng kết thúc ưu đãi</span>
                                {(initialData?.discountRate > 0 && !initialData?.discountEndDate) && (
                            ...
                            <input
                                className="glass" type="month"
                                style={{ width: '100%', padding: '0.75rem', boxSizing: 'border-box' }}
                                value={formData.discountEndDate || (initialData?.discountRate > 0 ? '2026-07' : '')}
                                onChange={e => setFormData({ ...formData, discountEndDate: e.target.value })}
                            />
                            <p style={{ fontSize: '0.75rem', color: 'var(--text-secondary)', marginTop: '0.5rem' }}>
                                Ưu đãi sẽ có hiệu lực từ tháng nhập học đến hết tháng được chọn ở trên.
                            </p>
                        </div>
                    )}
